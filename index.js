...
class ServerlessPlugin {
  constructor(serverless, options) {
    ...
    this.commands = {
      ...
      distributionId: {
        ...
        lifecycleEvents: ['distributionId'],
      },
      domainInfo: {
  ...
  syncDirectory() {
    ...
    const args = ['s3', 'sync', `${appDir}/`, `s3://${appBucket}/`, '--delete']
  ...
  async distributionId() {
    ...
    const distributionIdOutputKey =
      this.serverless.variables.service.custom.spa.distributionIdOutputKey
    ...
    const output = outputs.find(entry => entry.OutputKey === distributionIdOutputKey)
  ...
  async invalidateCache() {
    ...
    if (distributionId) {
      this.serverless.cli.log(`Invalidating CloudFront distribution with ID: ${distributionId}`)
      const args = [
        'cloudfront',
        'create-invalidation',
        '--distribution-id',
